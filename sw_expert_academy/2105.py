# 한변의 길이가 N인 정사각형
# 카페 사이에는 대각선의 길 있음
# 카페에서 출발하여 대각선 방향으로 사각형 모양 그리며 돌아와야함
# 같은 디저트를 팔고 있으면 안됨
# 왔던 길 다시 갈 수 없음
# 임의의 카페에서 출발하여 움직인다
# 가장 많이 먹을 수 있는 경로 찾고 그때의 디저트 수 출력
# 먹을 수 없는 경우 -1

dx = [1, 1, -1, -1]
dy = [1, -1, -1, 1]

def dfs(eat, start, x, y, direct):
    global result
    ...
    if direct == 3 and [x, y] == start:
        result = max(result, len(eat))
        return
    if dessert[x][y] in eat:
        return

    eat.append(dessert[x][y])
    nx = x + dx[direct]
    ny = y + dy[direct]
    dfs(eat, start, nx, ny, direct)
    eat.pop()
